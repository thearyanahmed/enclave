    ...
    pub async fn execute(&self, token: &str, new_password: &str) -> Result<(), AuthError> {
        ...
        match reset_token {
            Some(reset_token) => {
                ...
                self.user_repository.update_password(reset_token.user_id, &hashed).await?;
                self.reset_repository.delete_reset_token(token).await?;
...
mod tests {
    ...
    async fn test_reset_password_success() {
        ...
        let token = reset_repo.create_reset_token(user_id, expires_at).await.unwrap();
        ...
        let found = action.reset_repository.find_reset_token(&token.token).await.unwrap();
