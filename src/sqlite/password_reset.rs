...
impl PasswordResetRepository for SqlitePasswordResetRepository {
    ...
    async fn create_reset_token(
        ...
        let row: ResetTokenRecord = sqlx::query_as(
            "INSERT INTO password_reset_tokens (token_hash, user_id, expires_at) VALUES (?, ?, ?) RETURNING token_hash, user_id, expires_at, created_at",
        )
        .bind(&token_hash)
        .bind(user_id)
        .bind(expires_at)
    ...
    async fn find_reset_token(&self, token: &str) -> Result<Option<PasswordResetToken>, AuthError> {
        ...
        let row: Option<ResetTokenRecord> = sqlx::query_as(
            "SELECT user_id, expires_at, created_at FROM password_reset_tokens WHERE token_hash = ?",
        )
        .bind(&token_hash)
