...
impl EmailVerificationRepository for SqliteEmailVerificationRepository {
    ...
    async fn create_verification_token(
        ...
        let row: VerificationTokenRecord = sqlx::query_as(
            "INSERT INTO email_verification_tokens (token_hash, user_id, expires_at) VALUES (?, ?, ?) RETURNING token_hash, user_id, expires_at, created_at",
        )
        .bind(&token_hash)
        .bind(user_id as i64)
        .bind(expires_at)
    ...
    async fn find_verification_token(
        ...
        let row: Option<VerificationTokenRecord> = sqlx::query_as(
            "SELECT user_id, expires_at, created_at FROM email_verification_tokens WHERE token_hash = ?",
        )
        .bind(&token_hash)
